...
from data_helper import Wisdm_ar_data
...
class CNNEncoder(nn.Module):
    def __init__(self, t_size):
        ...
        self.feature_10 = nn.Sequential(
            nn.Conv1d(in_channels=3, out_channels=128, kernel_size=(7, ), padding=3, bias=False),
            nn.BatchNorm1d(128),
            nn.ReLU(),
    ...
    def forward(self, z, with_argumentation=False, with_connection=True):
        ...
        if with_argumentation:
            data = data_argumentation(data.detach(), series_length=128, sub_seq_length=32)
            data = data_argumentation(data.detach(), series_length=128, sub_seq_length=32)
...
class VAENew(nn.Module):
    ...
    def init_center_c(self, train_loader):
        n_samples = torch.ones(self.y_dims).cuda()
...
if __name__ == '__main__':
    ball_loss_weight = 100.0
    discriminator_loss = 10.0
    ...
    for v1, v2 in zip([9, 31, 2, 7, 4, 22, 6, 27, 15, 16],
                      [18, 11, 6, 25, 27, 8, 23, 20, 19, 7]):
        data_train = Wisdm_ar_data([v1])
